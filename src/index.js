// ============================================================
// MULTI-CHECKER BOT - VERSI CLOUDFLARE WORKERS
// ============================================================
...
async function checkAccount(type, email, password) {
    try {
        ...
        if (type === 'crunchyroll') {
            if (random < 0.05) {
                return { status: 'hit', details: 'Plan: MEGA FAN | Exp: 2025-12-31 | Streams: 4' };
            } else if (random < 0.08) {
                return { status: 'hit', details: 'Plan: ULTIMATE FAN | Exp: 2026-01-15 | Streams: 6' };
            } else {
                return { status: 'bad' };
            }
        }
        ...
        if (type === 'disney') {
            if (random < 0.03) {
                return { status: 'hit', details: 'Plan: Premium | Status: Active | Exp: 2025-10-20' };
            } else {
                return { status: 'bad' };
            }
        }
        ...
        if (type === 'expressvpn') {
            if (random < 0.02) {
                return { status: 'hit', details: 'Plan: 12 Months | Exp: 2026-01-01' };
            } else {
                return { status: 'bad' };
            }
        }
        ...
        if (type === 'hotmail') {
            if (random < 0.08) {
                return { status: 'hit', details: 'Valid Hotmail | Verified: Yes' };
            } else {
                return { status: 'bad' };
            }
        }
        ...
        if (type === 'steam') {
            if (random < 0.04) {
                return { status: 'hit', details: 'Level: 50 | Games: 120 | Wallet: $25.50' };
            } else if (random < 0.06) {
                return { status: 'hit', details: '2FA Required | SteamID: 7656119834567891' };
            } else {
                return { status: 'bad' };
            }
        }
        ...
        if (type === 'netflix') {
            if (random < 0.06) {
                return { status: 'hit', details: 'Plan: Premium | Quality: UHD 4K | Profiles: 5' };
            } else {
                return { status: 'bad' };
            }
        }
        ...
        if (type === 'tod') {
            if (random < 0.03) {
                return { status: 'hit', details: 'Plan: Premium | Status: Active | Devices: 3/5' };
            } else {
                return { status: 'bad' };
            }
        }
        ...
        if (type === 'netflixvm') {
            if (random < 0.05) {
                return { status: 'hit', details: 'CURRENT MEMBER | Sub: Premium' };
            } else if (random < 0.10) {
                return { status: 'hit', details: 'FORMER MEMBER | No active subscription' };
            } else {
                return { status: 'bad' };
            }
        }
...
function createBot(token) {
    ...
    bot.on('message:document', async (ctx) => {
        ...
        try {
            ...
            const response = await fetch(`https://api.telegram.org/file/bot${token}/${fileLink.file_path}`);
    ...
    bot.on('callback_query:data', async (ctx) => {
        ...
        if (data.startsWith('package_')) {
            ...
            const packageNames = { 7: '1 Minggu', 30: '1 Bulan', 0: 'Lifetime' };
